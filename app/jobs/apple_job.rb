...
Dashing.scheduler.every '5s', :first_in => 0.4 do
  ...
  unless response.nil?
    ...
    open_close= [
        {
            ...
            label: 'Close',
            data: ts_close,
            backgroundColor: [ 'rgba(255, 0, 132, 0.2)' ] * labels.length,
            borderColor: [ 'rgba(255, 0, 132, 1)' ] * labels.length,
            borderWidth: 1,
        }
    ]
    volume = [
    ...
    Dashing.send_event('apple_open_close', {labels: labels, datasets: open_close})
    Dashing.send_event('apple_volume', {labels: labels, datasets: volume})
  end
end
